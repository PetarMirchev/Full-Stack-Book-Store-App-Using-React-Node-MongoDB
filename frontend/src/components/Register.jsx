...
const Register = () => {
  ...
  const onSubmit = async (data) => {
    //console.log(data); //send data from form
    try {
      await registerUser(data.email, data.password);
      alert("User registration successful!");
      navigate("/login");
    } catch (error) {
      setMassage("Please provide a valid email and password");
      console.error(error);
    }
  };
  ...
  return (
    ...
    <div className='w-full max-w-sm mx-auto bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4'>
      ...
      <p className='align-baseline font-medium mt-4 text-sm'>You have a account? Please <Link to="/login" className='text-blue-500 hover:text-blue-700'>Login</Link></p>
